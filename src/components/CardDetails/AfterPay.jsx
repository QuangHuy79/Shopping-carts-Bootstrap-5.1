...
//       setShowThanks(true);
//       setTimeout(() => setShowThanks(false), 3000); // ✅ di chuyển vào đây
//       clearCart();
//       resetForm();
//     } catch (error) {
//       alert("❌ Gửi đơn hàng thất bại!");
...
import { AfterPayformikProps } from "./FormikProps";
function AfterPay() {
  const navigate = useNavigate();
  const { initialValues, validationSchema } = AfterPayformikProps;
  const { cartItems, clearCart } = useCart();
  const [showThanks, setShowThanks] = useState(false);
  const subtotalFromCart = cartItems.reduce(
    (sum, item) => sum + item.price * item.quantity,
        ...
        <div className="card-body">
          ...
          <FormikWrapper
            ...
            onSubmit={async (values, actions) => {
              ...
              console.log("Dữ liệu chuẩn bị gửi lên API:", orderData); // <-- đây nhé
              try {
                await submitOrder(orderData);
          ...
          >
            {({ values, errors, touched, handleChange }) => {
              ...
              const shipping =
                values && values.shippingMethod === "express"
                  ? 20
                  : values && values.shippingMethod === "standard"
                  ? 10
                  : 0;
              const total = subtotal + shipping;
